...
gulp.task('gen-package', ['copy-index'], function () {
  return gulp.src(['./package.json']).on('end', function (cb) {
    ...
    exec('cd dist &&YARN_IGNORE_ENGINES=true  yarn install --prod && yarn autoclean --force', cb);
  });
});

gulp.task('remove-redundant', ['gen-package'], function () {
  return del.sync(['dist/node_modules/protobufjs/dist', 'dist/node_modules/aliyun-sdk/dist']);
});
...
gulp.task('watch', function () {
  ...
  gulp.watch(['app/**/*'], function (event) {
    ...
    if (event.path.endsWith('.html') && event.path != path.join(__dirname, 'app/index.html'))
      taskFns.templates();
